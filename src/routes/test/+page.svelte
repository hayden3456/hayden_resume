    ...
    gsapScript.onload = () => {
      ...
      scrollTriggerScript.onload = () => {
        ...
        motionPathScript.onload = () => {
          ...
          // Helper function for smooth path movement
          function pathEase(path, config={}) {
            let axis = config.axis || "y",
                precision = config.precision || 1,
                rawPath = window.MotionPathPlugin.cacheRawPathMeasurements(window.MotionPathPlugin.getRawPath(gsap.utils.toArray(path)[0]), Math.round(precision * 12)),
                useX = axis === "x",
                ...
                i = 1;
            for (; i < l; i++) {
              positions[i] = (window.MotionPathPlugin.getPositionOnPath(rawPath, i / l)[axis] - start) / range;
            }
            positions[l] = 1;
            for (i = 0; i < l; i++) {
              getClosest(i / l);
          ...
          let rotateTo = gsap.quickTo(ball, "rotation");
          let prevDirection = 0;
          ...
          gsap.to(motionSVG, {
            scrollTrigger: {
              ...
              scrub: 0.5, // Reduced scrub for better responsiveness with pathEase
              markers: true,
              onUpdate: self => {
            ...
            ease: pathEase(motionPath, { smooth: 15 }), // Custom ease with increased smoothing for very smooth movement
...
<div class="min-h-screen bg-gradient-to-b from-[#fcedd1] via-[#fae4c9] via-[#f8d8c1] via-[#f7c6b1] to-slate-800">
  ...
  <section class="qualification-section-reverse">
    ...
    <div >
      <img src="/images/cicd.webp" alt="Learning & Growth" style="width: 100%; height: 80%; object-fit: cover; border-radius: 8px;" />
    </div>
  </section>
...
>
  ...
  <style type="text/css">
    .st0{fill:none;stroke:url(#pathGradient);stroke-width:1;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:10;}
  </style>
  <path 
    bind:this={motionPath}
    id="motionPath" 
    class="st0" 
    d="M45,20 L45,24 L50,25 L50,30 L36,31 L36,32 L95,50 L95,60 L1,60 L1,70 L95,70 L95,80 L1,80 L1,90 L95,90 L95,100 L1,100" 
  />
  
    <g bind:this={motionSVG} id="motionSVG">
    <g bind:this={ball} id="ball">
