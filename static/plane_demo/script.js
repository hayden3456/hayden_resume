...
class Scene
{
  ...
  {_defineProperty(this, "render",
    ...
    this.scene = new THREE.Scene();for (var _ii = 0; _ii < this.views.length; ++_ii) {var _view = this.views[_ii];var _camera = new THREE.PerspectiveCamera(65, window.innerWidth / window.innerHeight, 1, 2000);_camera.position.fromArray([0, 0, 180]);_camera.layers.disableAll();_camera.layers.enable(_ii);_view.camera = _camera;_camera.lookAt(new THREE.Vector3(0, 5, 0));} //light
...
function loadModel()
{
  ...
  loader.load('/aiaa/pretty_render_no_glider.obj', function (obj) {object = obj;});
}
...
function setupAnimation(model)
{
  ...
  gsap.set(plane.position, { x: 80, y: -32, z: 20 });
